...
class TB1Hub(QObject):
    ...
    def initUI(self):
        ...
        :return:
        '''
        print("初始化Table1 UI界面")

        self.mainWin.tb1.setRowCount(20)
        self.mainWin.tb1.setColumnCount(367)

        self.mainWin.tb1.setHorizontalHeaderLabels([str(i) for i in range(1, 368)])

        self.mainWin.tb1.setVerticalHeaderLabels([
            '期','1','2','3','4','5','6','7','胜','负','总','=','=','1','2','3','4','5','6','7'
        ])

        # 禁止调整列宽、行高
        self.mainWin.tb1.horizontalHeader().setSectionResizeMode(QHeaderView.Fixed)
        self.mainWin.tb1.verticalHeader().setSectionResizeMode(QHeaderView.Fixed)

        self.mainWin.tb1.horizontalHeader().setStyleSheet("QHeaderView::section{background-color:#dcdcdc;}")
        self.mainWin.tb1.verticalHeader().setStyleSheet("QHeaderView::section{background-color:#dcdcdc;}")

        # 右键、复制
        self.mainWin.tb1.customContextMenuRequested.connect(self.showContextMenu)
        self.set_shortcut()

        # 初始化所有Cell
        qFont = QFont()
        qFont.setBold(True)
        for row in range(0,self.mainWin.tb1.rowCount()):
            for col in range(0,self.mainWin.tb1.columnCount()):
                ...
                self.mainWin.tb1.setItem(row, col, itemWidget)
    ...
    def resetCells(self):
        ...
        for row in range(0,self.mainWin.tb1.rowCount()):
            # self.mainWin.tb1.setColumnHidden(row,True)
            for col in range(0,self.mainWin.tb1.columnCount()):
                self.mainWin.tb1.item(row,col).setText("")
                self.mainWin.tb1.item(row,col).setBackground(QBrush(defaultColor))
    ...
    def parse(self,df):
        ...
        # 推送计算结果
        self.push.emit(1,tmpDf)

        return tmpDf

    def parseHistogram(self,df):
        '''
        数据分布分析
        :return:
        '''
    ...
    def resetTbColumn(self):
        ...
        for col in range(0,count):
            self.mainWin.tb1.setColumnHidden(col,False)


        # self.mainWin.tb1.setColumnCount(count)
        # self.mainWin.tb1.setHorizontalHeaderLabels(labels)
    ...
    def resetItems(self):
        ...
        for index,row in self.resultDf.iterrows():
            ...
            item = self.mainWin.tb1.horizontalHeaderItem(index)
            item.setText(weekdays[w])

            if w == 0:
                self.mainWin.tb1.item(0, index).setForeground(QBrush(red))
            else:
                self.mainWin.tb1.item(0,index).setForeground(QBrush(black))

            # 期号
            self.mainWin.tb1.item(0, index).setText(str(row['issue']))

            # 显示列
            self.mainWin.tb1.setColumnHidden(index, False)

            # 是否最后一条记录？
            if index+1 >= len(self.resultDf):
                print("这是最后一条记录，显示预测结果")

                self.mainWin.tb1.item(0, index).setBackground(QBrush(yellow))
                ...
                self.mainWin.tb1.item(0, index).setBackground(QBrush(white))
            ...
            for n in range(1,8):
                ...
                if val == 1:
                    text = '大'
                    self.mainWin.tb1.item(n,index).setBackground(QBrush(da))
                elif val == -1:
                    text = '小'
                    self.mainWin.tb1.item(n,index).setBackground(QBrush(xiao))
                else:
                    text = '和'
                    self.mainWin.tb1.item(n,index).setBackground(QBrush(ping))

                self.mainWin.tb1.item(n, index).setText(text)


            # 胜
            self.mainWin.tb1.item(8, index).setText(str(row['win']))

            # 负
            lose = str(row['lose'])
            # if row['lose'] > 0:
            #     lose = '-'+lose
            self.mainWin.tb1.item(9, index).setText(lose)

            # 合计
            diff = row['win'] + row['lose']
            self.mainWin.tb1.item(10, index).setText(str(diff))
            if diff < 0:
                self.mainWin.tb1.item(10, index).setBackground(QBrush(red))
            else:
                self.mainWin.tb1.item(10, index).setBackground(QBrush(white))
            ...
            for n in range(1,8):
                ...
                if row[tk] == 0:
                    ...
                    self.mainWin.tb1.item(n+12, index).setText(str(row['t%d' % n]))
    ...
    def showContextMenu(self,pos):
        ...
        # 检查是否已选择内空
        if len(self.mainWin.tb1.selectedRanges()) == 0:
            copyItem.setEnabled(False)

        # 当前选择行号
        currentRow = self.mainWin.tb1.currentRow()
        print("currentRow = %d" % currentRow)

        # 被阻塞
        action = menu.exec_(self.mainWin.tb1.mapToGlobal(pos))
    ...
    def slot_copy(self):
        ...
        selectedRanges = self.mainWin.tb1.selectedRanges()
        ...
        for row in range(selectedRange.rowCount()):
            ...
            for col in range(selectedRange.columnCount()):
                ...
                item = self.mainWin.tb1.item(rowAt,colAt)

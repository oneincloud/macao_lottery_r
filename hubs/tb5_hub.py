...
class TB5Hub(QObject):
    ...
    def initUI(self):
        ...
        :return:
        '''
        print("初始化Table5 UI界面")

        self.mainWin.tb5.setRowCount(20)
        self.mainWin.tb5.setColumnCount(367)

        self.mainWin.tb5.setHorizontalHeaderLabels([str(i) for i in range(1, 368)])

        self.mainWin.tb5.setVerticalHeaderLabels([
            '期','1','2','3','4','5','6','7','胜','负','总','=','=','1','2','3','4','5','6','7'
        ])

        # 禁止调整列宽、行高
        self.mainWin.tb5.horizontalHeader().setSectionResizeMode(QHeaderView.Fixed)
        self.mainWin.tb5.verticalHeader().setSectionResizeMode(QHeaderView.Fixed)

        self.mainWin.tb5.horizontalHeader().setStyleSheet("QHeaderView::section{background-color:#dcdcdc;}")
        self.mainWin.tb5.verticalHeader().setStyleSheet("QHeaderView::section{background-color:#dcdcdc;}")

        # 右键、复制
        self.mainWin.tb5.customContextMenuRequested.connect(self.showContextMenu)
        self.set_shortcut()

        # 初始化所有Cell
        qFont = QFont()
        qFont.setBold(True)
        for row in range(0, self.mainWin.tb5.rowCount()):
            for col in range(0, self.mainWin.tb5.columnCount()):
                ...
                self.mainWin.tb5.setItem(row, col, itemWidget)

                self.mainWin.tb5.setColumnHidden(col, True)
    ...
    def resetCells(self):
        ...
        for row in range(0,self.mainWin.tb5.rowCount()):
            # self.mainWin.tb5.setColumnHidden(row,True)
            for col in range(0,self.mainWin.tb5.columnCount()):
                self.mainWin.tb5.item(row,col).setText("")
                self.mainWin.tb5.item(row,col).setBackground(QBrush(defaultColor))
    ...
    def resetTbColumn(self):
        ...
        if self.mainWin.tb5.columnCount != count:
            self.mainWin.tb5.setColumnCount(0)

        self.mainWin.tb5.setColumnCount(count)
        self.mainWin.tb5.setHorizontalHeaderLabels(labels)
    ...
    def resetItems(self):
        ...
        for index,row in self.resultDf.iterrows():
            ...
            item = self.mainWin.tb5.horizontalHeaderItem(index)
            item.setText(weekdays[w])

            if w == 0:
                self.mainWin.tb5.item(0, index).setForeground(QBrush(red))
            else:
                self.mainWin.tb5.item(0, index).setForeground(QBrush(black))
            ...
            for n in range(1, 8):
                ...
                if val == 1:
                    text = '大'
                    self.mainWin.tb5.item(n, index).setBackground(QBrush(da))
                elif val == -1:
                    text = '小'
                    self.mainWin.tb5.item(n, index).setBackground(QBrush(xiao))
                else:
                    text = '和'
                    self.mainWin.tb5.item(n, index).setBackground(QBrush(ping))

                self.mainWin.tb5.item(n, index).setText(text)

            # 期号
            self.mainWin.tb5.item(0, index).setText(str(row['issue']))

            # 显示列
            self.mainWin.tb5.setColumnHidden(index, False)

            # 是否最后一条记录？
            if index + 1 >= len(self.resultDf):
                print("这是最后一条记录，显示预测结果")

                self.mainWin.tb5.item(0, index).setBackground(QBrush(yellow))
                ...
                for n in range(1, 8):
                    ...
                    if val > 0:
                        ...
                        self.mainWin.tb5.item(n, index).setText('')
                        self.mainWin.tb5.item(n + 12, index).setText('')
                break  # 显示完最后一列预测结果
            else:
                self.mainWin.tb5.item(0, index).setBackground(QBrush(white))

            # 胜
            self.mainWin.tb5.item(8, index).setText(str(row['win']))

            # 负
            lose = str(row['lose'])
            # if row['lose'] > 0:
            #     lose = '-' + lose
            self.mainWin.tb5.item(9, index).setText(lose)

            # 合计
            diff = row['win'] + row['lose']
            self.mainWin.tb5.item(10, index).setText(str(diff))
            if diff < 0:
                self.mainWin.tb5.item(10, index).setBackground(QBrush(red))
            else:
                self.mainWin.tb5.item(10, index).setBackground(QBrush(white))
            ...
            for n in range(1, 8):
                ...
                if row[tk] == 0:
                    ...
                    self.mainWin.tb5.item(n + 12, index).setText(str(row['t%d' % n]))
    ...
    def showContextMenu(self,pos):
        ...
        # 检查是否已选择内空
        if len(self.mainWin.tb5.selectedRanges()) == 0:
            copyItem.setEnabled(False)

        # 当前选择行号
        currentRow = self.mainWin.tb5.currentRow()
        print("currentRow = %d" % currentRow)

        # 被阻塞
        action = menu.exec_(self.mainWin.tb5.mapToGlobal(pos))
    ...
    def slot_copy(self):
        ...
        selectedRanges = self.mainWin.tb5.selectedRanges()
        ...
        for row in range(selectedRange.rowCount()):
            ...
            for col in range(selectedRange.columnCount()):
                ...
                item = self.mainWin.tb5.item(rowAt,colAt)
